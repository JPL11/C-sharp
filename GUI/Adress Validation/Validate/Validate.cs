   ...
   public partial class ValidateForm : Form
   {
      ...
      private void okButton_Click( object sender, EventArgs e )
      {
         ...
         else
         {
               ...
               /* else if (!ValidateInput(zipCodeTextBox.Text,        // Part B
                ...
                { 
                    {
                            try  //Part B
                            {
                                MessageBox.Show("Thank You!\nPlease check the inputted data if applicable", "Information Correct",
                           MessageBoxButtons.OK, MessageBoxIcon.Information);
                            ...
                            {
                                if (!ValidateInput(zipCodeTextBox.Text,
                                         @"^\d{9}$", "Invalid zip code") || !ValidateInput(zipCodeTextBox.Text,
                                         @"^\d{5}$", "Invalid zip code") || !ValidateInput(zipCodeTextBox.Text,
                                         @"^[1-9]\d{4}-\d{4}$", "Invalid zip code"))
                                    zipCodeTextBox.Select();

                            } 
                    }
                /*    //this.Hide(); // hide main window   (Part A: Omit this line only)
